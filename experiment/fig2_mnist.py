DOCS = """
train the model on the a dataset (gabor or mnist) or show the model learns to represent the data and predict the input
Current figures: figure 2.
"""
...
if __name__ == '__main__':
    ...
    # dataset args
    parser.add_argument('--digit_mode', type=str, default='variable')
    ...
    logger = Logger('experiment for figure 2 / mnist dataset')
    ...
    checkpointer = Checkpointer(net, 'mnist', 'shallow_mlp', 'fig2', ['seed'], args, sub_dir="", logger=logger)
    ...
    digit_dataset = Digit(digit_mode=args.digit_mode, n_samples=2000, topdown=args.topdown_mode, noise=args.noise)
    dataloader = DataLoader(digit_dataset, batch_size=args.batch_size, shuffle=True)
