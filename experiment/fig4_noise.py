DOCS = """
train the model on the a dataset (gabor or mnist) and show the robustness of the model to noise.
currently figure 4.
"""

from dataset import GaborDatasetOrientation
from model import ShallowMLP, DenoisingAE
from experiment.utils import set_seed, init_model, Logger, Trainer, AETrainer, Checkpointer
from dataset.utils import NoisyDataset
import torch
...
if __name__ == '__main__':
    ...
    parser.add_argument('--denoising_ae', action='store_true')
    ...
    # experiment args
    parser.add_argument('--added_noise', type=float, required=True)
    ...
    logger = Logger('experiment for figure 4 / noise')
    ...
    logger.verbose(f'added noise: {args.added_noise}', 'red')
    
    if args.denoising_ae:
        logger.verbose('Using denoising autoencoder', 'blue')
    ...
    checkpointer = Checkpointer(net, 'gabor', 'shallow_mlp', 'fig4/noise', ['seed','ablate_thal_l5','ablate_delay','ablate_topdown','ablate_l23_l5', 'added_noise'], args, sub_dir="ae" if args.denoising_ae else "", logger=logger)
    ...
    dataloader_original = DataLoader(gabor_dataset, batch_size=1000, shuffle=True)
    x1_sample, x2_sample, td_sample, _, _ = next(iter(dataloader_original))
    ...
    if args.denoising_ae:
        trainer = AETrainer(net, dataloader, optimizer, args, logger, device)
        logger.verbose('training autoencoder ...', 'blue')
