DOCS = """
train the model on the a dataset (gabor or mnist) to show the effect of feedback connection from L5 to L2/3.
Current figures: figure 5.
"""
...
if __name__ == '__main__':
    ...
    parser.add_argument('--fa_sparsity', type=float, required=True)
        
    args = parser.parse_args()
    set_seed(args.seed)

    ssl_coeff = 1. if not args.ablate_l23_l5 else 0.
    logger = Logger('experiment for figure 6')
    
    logger.verbose(vars(args), 'magenta')
    
    logger.verbose(f'l23_l5_fa: {args.l23_l5_fa}', 'red')
    logger.verbose(f'fa_sparsity: {args.fa_sparsity}', 'red')
    ...
    checkpointer = Checkpointer(net, 'gabor', 'shallow_mlp', 'fig6', ['seed','l23_l5_fa','fa_sparsity'], args, sub_dir="", logger=logger)
    ...
    dataloader = DataLoader(gabor_dataset, batch_size=32, shuffle=True)
