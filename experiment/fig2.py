...
if __name__ == '__main__':
    ...
    logger = Logger('experiment for figure 2')
    
    logger.verbose(vars(args), 'magenta')
    
    net = init_model(ShallowMLP,  **vars(args), logger=logger)
    
    # freeze thal-l4 weights
    if args.freeze_thal_l4:
        logger.verbose("FREEZING THAL-L4 WEIGHTS ", "red")
        for name, param in net.l4.named_parameters():
            logger.verbose(f"freezing param: {name}", "red")    
    ...
    sub_dir="freeze_thal_l4" if args.freeze_thal_l4 else ""
    checkpointer = Checkpointer(net, 'gabor', 'shallow_mlp', 'fig2', ['seed'], args, sub_dir=sub_dir, logger=logger)
